...
export default function Q6Screen({ navigation }: Props) {
  ...
  AsyncStorage.setItem('Q6', "2");

  const snapTo = async (val: number) => {
    const snapped = Math.round(val);
    setValue(snapped);
    try {
      await AsyncStorage.setItem('Q6', snapped.toString());
  ...
  return (
    <SafeAreaView style={styles.page}>
      <View style={styles.header}>
        <Text style={styles.headerText}>PHQ Questionnaire 6/9</Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        <NextButton targetScreen='Q7Screen' />
      </View>
    </SafeAreaView>
  );
}
